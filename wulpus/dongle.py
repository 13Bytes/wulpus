...
class WulpusDongle():
    ...
    def get_available(self) -> list[dict[str, str]]:
        ...
        Returns:
            list[dict[str, str]]: A list of {device_key: description}.
        """
        ports = comports()
        self._ports = sorted(ports)
        return list(map(lambda p: {p.device: p.description}, self._ports))
    ...
    def get_status(self):
        if self.__ser__.is_open:
            return f"Connected to {self.__ser__.port}"
        else:
